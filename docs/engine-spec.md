...
| 6 | 広域探索・実験 (`scenarios/frontier.ts`) | 資源の方向も初期位置も一切知らない状態で、フェロモンなしの分離則（Separation）による自己組織化的な拡散＋補給所建設だけで資源パッチ（資源が集中して存在する地点、4箇所配置）を発見・回収できるか | 資源を5個、拠点または補給所に搬入（180000tick以内） | 🧪 headless 10回中10勝（tick 542〜1431で勝利）。詳細はroadmap.md参照 |
