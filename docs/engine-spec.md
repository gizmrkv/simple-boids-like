...
用途で使われている。`ladder.ts`/`supply-line.ts`は追加で`memory[2]`を上記の
建設意思表示に使う。`frontier.ts`はさらに`memory[3]`を「この個体固有の探索
方位（ラジアン）」に使う——scenario側がspawn時に1回だけ書き込み、プログラム
自身は読むだけで書き換えない静的な値（詳細はシナリオ一覧の#6参照）。新しい
...
| 6 | 広域探索・実験 (`scenarios/frontier.ts`) | 資源の方向を一切知らない状態で、フェロモンなしの個体固有の固定方位分散＋補給所建設だけで資源クラスタを発見・回収できるか | 資源を5個、拠点または補給所に搬入（180000tick以内） | 🧪 headless 10回中7勝（tick 508〜1174で勝利、残り3回は資源クラスタに一度も遭遇できず時間切れ。詳細はroadmap.md参照） |
